...
data <- readRDS(file = "trainingData.rds")
...
saveRDS(weightsXTS, file = "trainingPortfolioWeights.rds")

#NEW
#equalWeightsXTS <- weightsXTS * 0.0 + 1/length(riskyAssetNames) 
#equalWeightsXTS$rf <- equalWeightsXTS$rf * 0.0 
#chart.StackedBar(equalWeightsXTS, date.format="%Y", cex.legend = 0.7, colorset=myColors(20)) 
#saveRDS(equalWeightsXTS, file = "trainingPortfolioEqualWeights.rds")
